...
export default function SearchEntity() {
  ...
  useEffect(() => {
    if (router.isReady) {
      const url = `/api/booksearch?includeAuthor=true&booklimit=20&bestsellered=true&skip=${
        (page - 1) * 20
      }`;
      const nextPageUrl = `/api/booksearch?includeAuthor=true&booklimit=20&&bestsellered=true&skip=${
        page * 20
      }`;
      const opts = {
        method: "GET",
  ...
  return (
    <Template>
      <div className="ml-24  text-2xl font-bold mt-14">Best Seller</div>
