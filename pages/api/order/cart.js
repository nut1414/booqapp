  ...
  async function cart(req, res) {
    ...
    try {
      ...
      if (!bookID) {
        ...
        const book = await prisma.bookdetails.findUnique({ where: { BookID: parseInt(bookID) } })
        ...
        const itembasket = await prisma.iteminbasket.findFirst({ where: { BookID: parseInt(bookID), UserID: req.user.UserID } })
        ...
        if (itembasket) {
          const item = await prisma.iteminbasket.update({
            ...
            data: {
              Quantity: quantity ? parseInt(quantity ? quantity : 1) : itembasket.Quantity + 1
            }
          })
      ...
      if (!itemID || !quantity) {
        ...
        const item = await prisma.book.findUnique({ where: { ItemID: parseInt(itemID), UserID: req.user.UserID } })
        ...
        const itemUpdated = await prisma.iteminbasket.update({ where: { ItemID: parseInt(itemID), UserID: req.user.UserID }, data: { Quantity: quantity } })
      ...
      if (!itemID) {
        ...
        const itemDeleted = await prisma.iteminbasket.delete({ where: { ItemID: parseInt(itemID), UserID: req.user.UserID } })
